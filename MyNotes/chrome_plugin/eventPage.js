...
function getPathAndAddBookMark(id, path, url, title) {
  ...
  var bookmarknode = chrome.bookmarks.get(id, function(bookmarknodes) {
	...
	if(typeof bookmarknodes[0].parentId !== 'undefined') {
		...
		var note = new myNotes.Note('/bookmarks' + path, title, url, true);
		/*	
		var payload = { }; 	
		payload = {
...
function getPathAndRemoveBookMark(id, path, title) {
  ...
  var bookmarknode = chrome.bookmarks.get(id, function(bookmarknodes) {
	...
	if(typeof bookmarknodes[0].parentId !== 'undefined') {
	    ...
	    console.log('Path:' + path);
		var myNotes = new MyNotesFunc();
		var note = new myNotes.Note('/bookmarks' + path, title);
		...
		var settings = {};
   		...
   		send_ajax_post('https://mynotes.io/remove', 'application/json',  JSON.stringify(payload), settings, handle_remove_notes_success, handle_remove_notes_error);
		*/
